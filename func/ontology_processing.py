...
def domain_knowledge_processing_partitioning(query):
    ...
    grades.drop(
        ["code_module", "code_presentation", "id_student", "weight", "id_assessment"],
        axis=1,
        inplace=True,
    )
    ...
    grades = grades.groupby(["case:concept:name"]).agg(
        {"score": "sum", "still_enrolled": "sum", "count": "count"}
    )
...
def domain_knowledge_clustering(query_grades, query_vle):
    ...
    try:
        ...
        vle = cursor.fetchall()
        logger.info(
            "Information read-in. vle has length %s and grades have length %s",
            len(grade_profile),
            len(vle),
        )
